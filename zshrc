...
plugins=(git common-aliases zsh-autosuggestions zsh-bitbake)
...
# If we're in WSL2, grab the host IP address
...
prompt_git() {
  ...
  local ref dirty mode repo_path prompt_bg prompt_fg
  ...
  if [[ "$(git rev-parse --is-inside-work-tree 2>/dev/null)" = "true" ]]; then
    ...
    prompt_segment $prompt_bg $prompt_fg
...
if [[ -n $EXTRA_RC_FILE && -f "$EXTRA_RC_FILE" ]]; then
    source $EXTRA_RC_FILE
fi
